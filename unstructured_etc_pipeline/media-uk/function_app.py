...
async def cosmosdb_trigger(documents: func.DocumentList):
    try:
        from media.proc_media_cosmos import run_cosmos_media
        ...
        for doc in documents:
            ...
            if doc_data['next_action'] == True:
                await run_cosmos_media(doc_data)
...
async def HttpTrigger(req: func.HttpRequest, context: func.Context) -> func.HttpResponse:
    ...
    try:
        ...
        from media.proc_media_timer import run_media, get_seed_count
                
        EMREGENCY_VERSION =os.environ.get('EMREGENCY_VERSION', await get_last_version())

        LAST_VERSION = EMREGENCY_VERSION # 정상 배치의 경우 6자리 날짜(ex. 250613), 긴급 배치의 경우 시,분 포함 10자리(ex. 2506131509)
        ...
        if not is_running:
            ...
            success = await run_media((LAST_VERSION, CURRENT_VERSION))
