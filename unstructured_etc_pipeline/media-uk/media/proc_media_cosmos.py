...
from media.proc_media_common import MediaSearch, MediaSeedContainer, MediaRefData
from media.common_utils import generate_id, generate_original_id, generate_filter_id
...
async def run_cosmos_media(doc_data):
    seed = SeedData(**doc_data)
    ref_data = MediaRefData(**seed.ref_data)

    await change_status(SYSTEM_NAME,TaskStatus.IN_PROGRESS,TaskType.DEFAULT,seed,MediaSeedContainer)
    ...
    try:
        ...
        await LogManager.error(f"qset insert Error : {str(e)}")
    ...
    try:
        index = MediaSearch(
            ...
            is_display = ref_data.is_display,
            ...
            reg_date = ref_data.reg_date,
            disp_strt_dtm = ref_data.disp_strt_dtm,
            disp_end_dtm = ref_data.disp_end_dtm,
            img_data = ref_data.img_data,
        )
        ...
        try:
            ...
            await LogManager.exception(f"Index Upload Error : {str(e)}")
            await change_status(SYSTEM_NAME,TaskStatus.ERROR,TaskType.DEFAULT,seed,MediaSeedContainer)
        
    except Exception as e:
        await LogManager.exception(e,"cosmos trigger error")
        await change_status(SYSTEM_NAME,TaskStatus.ERROR,TaskType.DEFAULT,seed,MediaSeedContainer)
    
    await change_status(SYSTEM_NAME,TaskStatus.COMPLETED,TaskType.DEFAULT,seed,MediaSeedContainer)
